...
import {
  ...
  deserializeSequenceStep,
  getActiveStep,
} from '@/lib/sequencer/engine';
...
function useRealSequencer() {
  ...
  const createPlan = useCallback(
    async (templateId: TemplateId, params: TemplateParams) => {
  ...
  const simulateStep = useCallback(
    async (stepId: string): Promise<SimulationResult> => {
      ...
      if (unmetDeps.length > 0) {
        throw new Error(
          `Cannot simulate step ${stepId}: unmet dependencies ${unmetDeps.join(', ')}`,
        );
      }
          ...
          prev
            ? {
                ...
                steps: prev.steps.map((s) =>
                  s.id === stepId ? { ...s, status: 'simulating' } : s,
                ),
              }
            : null,
        );
  ...
  const executeStep = useCallback(
    async (stepId: string): Promise<string> => {
      const currentPlan = planRef.current;
      if (!currentPlan) throw new Error('No active plan');
      const step = currentPlan.steps.find((s) => s.id === stepId);
      if (!step) throw new Error('Step not found');
      ...
      if (unmetDeps.length > 0) {
        throw new Error(
          `Cannot execute step ${stepId}: unmet dependencies ${unmetDeps.join(', ')}`,
        );
      }
      ...
      try {
        ...
        const patchRes = await fetchWithTimeout(
          `/api/sequencer/plan/${currentPlan.id}/step/${stepId}`,
          {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
          ...
          setPlan((prev) =>
            prev
              ? {
                  ...prev,
                  steps: prev.steps.map((s) =>
                    s.id === stepId ? { ...s, status: 'confirmed', txHash } : s,
                  ),
                }
              : null,
          );
        ...
        setPlan((prev) =>
          prev
            ? {
                ...prev,
                steps: prev.steps.map((s) =>
                  s.id === stepId ? { ...s, status: statusToRevert } : s,
                ),
              }
            : null,
        );
  ...
  const signStep = useCallback(
    async (stepId: string, txHash: string): Promise<void> => {
      const currentPlan = planRef.current;
      if (!currentPlan) throw new Error('No active plan');
