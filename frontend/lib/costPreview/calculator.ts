...
import type { BridgeQuoteParams } from '@/types/shared';

/**
 * Calculate gas cost in USD for a given number of gas units on a chain.
 */
function calculateGasCostUsd(gasUnits: number, gasPriceGwei: number, ethPriceUsd: number): number {
...
async function getBridgeStepCosts(
  ...
  const asset =
    'token' in buildParams
      ? (buildParams as BridgeQuoteParams).token
      : 'asset' in buildParams
        ? (buildParams as { asset: string }).asset
        : 'USDC';
...
function computeDeleverageBreakEven(params: {
  ...
  const breakEvenDays =
    netAnnualUpliftUsd > 0 ? (params.totalCostUsd / netAnnualUpliftUsd) * 365 : Infinity;
...
export async function calculateCostPreview(
  ...
  if ('plan' in input) {
    ...
    const bridgeFeeUsd =
      input.sourceChain === input.destChain ? 0 : input.amountUsd * (isStable ? 0.0006 : 0.0012);
    const slippageUsd =
      input.amountUsd * (input.sourceChain === input.destChain ? 0.0005 : isStable ? 0.001 : 0.003);
    totalBridgeFeeUsd = bridgeFeeUsd;
    totalSlippageUsd = slippageUsd;
