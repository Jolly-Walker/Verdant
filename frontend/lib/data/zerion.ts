...
export function getVerdantProtocol(
  protocol: string | null,
  symbol: string,
  name: string,
): ProtocolId | null {
  ...
  if (p.includes('morpho') || n.includes('morpho') || s.includes('morpho') || s === 'hyperusdc')
    return 'morpho';
...
export async function fetchZerionPositions(address: string): Promise<Position[]> {
  ...
  try {
    const res = await fetch(`${ZERION_BASE}/wallets/${address}/positions/?${params}`, {
      headers: {
...
export function normaliseZerionPositions(raw: ZerionPosition[]): Position[] {
  return raw
    ...
    .filter(
      (p): p is Position => p !== null && (p.protocol !== 'wallet' || p.positionType === 'wallet'),
    );
}
