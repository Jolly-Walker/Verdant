...
import { getVerdantProtocol, normaliseZerionPositions, type ZerionPosition } from '../zerion';
...
describe('Zerion Data Normalization', () => {
  ...
  describe('normaliseZerionPositions', () => {
    ...
    it('should normalize staked/locked positions as LP', () => {
      const staked = createMockPosition({
        attributes: { position_type: 'staked', protocol: 'Pendle' },
      });
      const locked = createMockPosition({
        attributes: { position_type: 'locked', protocol: 'Pendle' },
      });
        ...
        createMockPosition({
          attributes: {
            fungible_info: {
              symbol: 'USDC',
              name: 'USD Coin',
              implementations: [{ chain_id: 'ethereum', address: null, decimals: 6 }],
            },
          },
        }),
      ];
