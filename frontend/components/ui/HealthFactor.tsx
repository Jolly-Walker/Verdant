...
export function HealthFactor({ value, maxScale = 3.0, showLabel = true }: HealthFactorProps) {
  ...
  const cautionWidthPct = Math.max(
    Math.min((CAUTION_MAX / maxScale) * 100, 100) - dangerWidthPct,
    0,
  );
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <p className={`font-mono text-sm font-medium tabular-nums ${textClass}`}>{displayValue}</p>
      </div>
    </div>
  );
}
