...
export function PositionList({
  ...
  const filteredPositions =
    filter === 'all'
      ...
      : positions.filter((p) => {
          if (filter === 'pendle')
            return p.positionType === 'pendle-pt' || p.positionType === 'pendle-yt';
          return p.positionType === filter;
        });

  const positionsByChain = chainIds
    .map((chainId) => {
      ...
      const protocolGroups = Object.entries(groupedByProtocol).map(
        ([protocolId, protocolPositions]) => {
          ...
          const healthFactor = protocolPositions.find(
            (p) => p.healthFactor !== undefined,
          )?.healthFactor;

          return {
            protocolId,
            displayName:
              PROTOCOL_NAMES[protocolId] ||
      ...
      <div className="flex flex-col items-center justify-center py-16 bg-verdant-surface border border-[#E5E0D8] border-dashed rounded-xl shadow-organic">
        <div className="h-12 w-12 rounded-full bg-verdant-surface-accent flex items-center justify-center mb-4">
          <svg
            ...
            className="text-verdant-text-muted"
          >
        ...
        <p className="text-verdant-text-muted text-sm max-w-sm text-center">
          You don&apos;t have any positions on supported protocols across{' '}
                ...
                <div className="bg-[#FAF9F6] border-b border-[#E5E0D8]/40 px-5 py-3.5 flex items-center justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] bg-verdant-surface-accent text-verdant-moss font-semibold px-2 py-0.5 rounded font-mono">
                      {protoGroup.positions.length}{' '}
                      {protoGroup.positions.length === 1 ? 'asset' : 'assets'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    {protoGroup.healthFactor !== undefined && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span
                          className={`font-mono text-xs font-bold ${getHealthFactorColor(protoGroup.healthFactor)}`}
                        >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`font-mono text-sm font-bold ${protoGroup.netValueUsd < 0 ? 'text-verdant-loss' : 'text-verdant-text-primary'}`}
                      >
                        {protoGroup.netValueUsd < 0 ? '-' : ''}
                        {formatUsd(Math.abs(protoGroup.netValueUsd))}
                      </span>
                    </div>
                  </div>
                </div>
