...
const PositionsQuerySchema = z.object({
  ...
  solana: z
    .string()
    .refine((addr) => isValidAddress(addr, 'solana'), { message: 'Invalid Solana address' })
    .optional(),
});
...
export async function GET(req: NextRequest) {
  ...
  try {
      ...
      for (const [pluginId, plugin] of Object.entries(PROTOCOL_REGISTRY)) {
        for (const chain of plugin.supportedChains) {
          ...
          protocolPositionsPromises.push(
            plugin.fetcher.fetchPositions(address, chain).catch((err) => {
              console.error(
                `[positions] failed to fetch positions for ${pluginId} on ${chain}:`,
                err,
              );
              return [];
            }),
    ...
    const allPositions = deduplicatePositions([
      ...evmPositions,
      ...solanaPositions,
      ...flatProtocolPositions,
    ]);
